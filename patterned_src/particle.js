class Particle {
    constructor(x, y, color, pattern = 'default') {
        this.x = x;
        this.y = y;
        ...
        this.pattern = pattern; // Determines the movement pattern
        this.depth = Math.random(); // Added for depth in circular patterns
    }
    ...
    update() {
        ...
        if (this.pattern === 'new_particle') {
            this.updateNewParticle();
        } else {
            this.updateDefault();
        }
    }
    ...
    updateNewParticle() {
        this.angle += this.angularSpeed * particleSpeed * (1 / (0.5 + this.depth));

        // Base radius for circular movement pattern
        const movementRadius = 20 * (1 / (0.1 + this.depth));

        // Calculate new position using a circular pattern
        this.x = this.originalX + movementRadius * Math.cos(this.angle);
        this.y = this.originalY + movementRadius * Math.sin(this.angle);
    }
